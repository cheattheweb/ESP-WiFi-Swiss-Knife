...
SSIDs::SSIDs() { list = new SimpleList<SSID>; }
...
void SSIDs::load() {
  ...
  for (uint32_t i = 0; i < arr.size() && i < SSID_LIST_SIZE; i++) {
    ...
    internal_add(tmpArray.get<String>(0), tmpArray.get<bool>(1),
                 tmpArray.get<int>(2));
  }
...
void SSIDs::save(bool force) {
  ...
  buf += String(OPEN_CURLY_BRACKET) + String(DOUBLEQUOTES) +
         str(SS_JSON_RANDOM) + String(DOUBLEQUOTES) + String(DOUBLEPOINT) +
         b2s(randomMode) + String(COMMA); // {"random":false,
  buf += String(DOUBLEQUOTES) + str(SS_JSON_SSIDS) + String(DOUBLEQUOTES) +
         String(DOUBLEPOINT) + String(OPEN_BRACKET); // "ssids":[
  ...
  for (int i = 0; i < c; i++) {
    ...
    buf += String(OPEN_BRACKET) + String(DOUBLEQUOTES) + name +
...
int SSIDs::getLen(int num) { return check(num) ? list->get(num).len : 0; }
...
void SSIDs::add(String name, bool wpa2, int clones, bool force) {
  ...
  if (clones > SSID_LIST_SIZE)
    clones = SSID_LIST_SIZE;
  ...
  if (accesspoints.selected() > 0) {
    ...
    for (int i = 0; i < apCount; i++) {
      if (accesspoints.getSelected(i))
        add(accesspoints.getSSID(i), accesspoints.getEnc(i) != 0, clones,
            force);
    }
  }
}

bool SSIDs::getRandom() { return randomMode; }
...
void SSIDs::print(int num) { print(num, true, false); }
...
int SSIDs::count() { return list->size(); }

bool SSIDs::check(int num) { return num >= 0 && num < count(); }
...
void SSIDs::internal_remove(int num) { list->remove(num); }

void SSIDs::internal_removeAll() { list->clear(); }