...
template <class T> struct SimpleListNode {
  T data;
...
template <typename T> SimpleList<T>::SimpleList(int (*compare)(T &a, T &b)) {
  setCompare(compare);
}

// Clear Nodes and free Memory
template <typename T> SimpleList<T>::~SimpleList() { clear(); }
...
template <typename T> SimpleListNode<T> *SimpleList<T>::getNode(int index) {
...
template <typename T> int SimpleList<T>::size() { return listSize; }

template <typename T> bool SimpleList<T>::isSorted() { return sorted; }

template <typename T> bool SimpleList<T>::isEmpty() { return listSize == 0; }

template <typename T> void SimpleList<T>::add(T obj) {
  // create new node
...
template <typename T> void SimpleList<T>::add(int index, T obj) {
  if ((index < 0) || (index >= listSize)) {
    return;
  }
...
template <typename T> void SimpleList<T>::insert(T obj) {
  if (!compare) {
    add(obj);
    return;
  }
  ...
  if (listSize == 0) {
    ...
    if (compare(obj, listEnd->data) >= 0) {
      ...
      // here a sequential search, because otherwise the previous node couldn't
      // be accessed
      while (h && !found) {
...
template <typename T> void SimpleList<T>::replace(int index, T obj) {
  if ((index >= 0) && (index < listSize)) {
    getNode(index)->data = obj;
  }
}

template <typename T> void SimpleList<T>::swap(int x, int y) {
  // only continue when the index numbers are unequal and at least 0
  if ((x != y) && (x >= 0) && (y >= 0)) {
    if (x > y) { // the first index should be smaller than the second. If not,
                 // swap them!
      int h = x;
      x = y;
      y = h;
...
template <typename T> void SimpleList<T>::remove(int index) {
  if ((index < 0) || (index >= listSize))
    return;
...
template <typename T> void SimpleList<T>::removeFirst() { remove(0); }

template <typename T> void SimpleList<T>::removeLast() { remove(listSize - 1); }

template <typename T> bool SimpleList<T>::has(T obj) {
  return binSearch(obj) >= 0;
}

template <typename T> int SimpleList<T>::count(T obj) {
  if (compare == NULL)
    return -1;
...
template <typename T> T SimpleList<T>::get(int index) {
...
template <typename T> T SimpleList<T>::getFirst() { return get(0); }

template <typename T> T SimpleList<T>::getLast() { return get(listSize - 1); }

template <typename T> void SimpleList<T>::moveToEnd() {
  SimpleListNode<T> *h = listBegin;
...
template <typename T> int SimpleList<T>::search(T obj) {
  if (compare == NULL)
    return -1;
...
template <typename T> int SimpleList<T>::searchNext(T obj) {
  if (compare == NULL)
    return -1;
...
template <typename T> int SimpleList<T>::binSearch(T obj) {
  return binSearch(obj, 0, listSize - 1);
}

template <typename T> T SimpleList<T>::pop() {
  T data = getLast();
...
template <typename T> T SimpleList<T>::shift() {
  T data = getFirst();
...
template <typename T> void SimpleList<T>::clear() {
  while (listSize > 0)
    removeFirst();
...
template <typename T> void SimpleList<T>::sort() {
  if (compare == NULL)
    return;
